...
There is no `first_name` / `last_name` mapping. For one coherent person, call `egyptian_name()` once and read `parts_ar` / `parts_en`. Source: [`faker-egy-names/`](faker-egy-names/) · [`faker-egy-names-php/`](faker-egy-names-php/) · [PyPI](https://pypi.org/project/faker-egy-names/) · [FakerPHP](https://github.com/AbdullahAfifyKhalil/faker-egy-names-php).
