...
class DataLoader {
  ...
  static DataBundle loadBundle({String? customPath}) {
    ...
    final rawYears = jsonMap['cohort_years'] as List<dynamic>? ?? [];
    final cohortYears = rawYears.map((y) => (y as num).toInt()).toList();

    final bundle = DataBundle(
      version: jsonMap['version'] as String? ?? '0.1.0',
      corpusTokens: (jsonMap['corpus_tokens'] as num? ?? 0).toInt(),
      corpusStudents: (jsonMap['corpus_students'] as num? ?? 0).toInt(),
